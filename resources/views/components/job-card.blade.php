...
<div class="rounded-lg shadow-md bg-white/5 p-4 text-white">
    ...
    <ul class="my-4 bg-white/10 p-4 rounded">
        <li class="mb-2"><strong>Salary:</strong> ${{number_format($job->salary)}}</li>
        <li class="mb-2">
            <strong>Location:</strong> {{$job->address}}, {{$job->city}}
            @if ($job->remote)
    ...
    <a
        ...
        class="block w-full text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium text-slate.-700 bg-white/20 hover:bg-white/25"
        >
        Details
    </a>
</div>
